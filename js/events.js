  ...
  const attachHandlers = (card, event, apiConfig, updateAvailability) => {
    ...
    if (buyForm instanceof HTMLFormElement) {
      buyForm.addEventListener('submit', async (evt) => {
        ...
        const holderName = String(new FormData(buyForm).get('holderName') || '').trim();
        const holderEmail = String(new FormData(buyForm).get('holderEmail') || '').trim();
        const phone = String(new FormData(buyForm).get('phone') || '').trim();
        const tierName = String(new FormData(buyForm).get('tierName') || '').trim();
        const quantity = Number(new FormData(buyForm).get('quantity') || 1);
